...
{
    Serial.begin(9600);
    EWD::connectToNetwork = false; //true=try to connect to router  false=go straight to hotspot mode (default: false)
    EWD::wifiRestartNotHotspot = true; //when connection issue, true=retry connection to router  false=fall back to hotspot (default: false)
    ...
    EWD::APName = "ESP"; //name of hotspot network (default: ESP32)
    ...
    EWD::hotspotAddress = IPAddress(10, 25, 21, 10); //if the esp32 creates a hotspot, what address should it give itself on its network (default: 10.25.21.1)
